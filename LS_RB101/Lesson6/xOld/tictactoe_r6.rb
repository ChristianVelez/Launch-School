...
def players(brd)
  test = { "player" => player_places_piece!(brd), "computer" => computer_places_piece!(brd)}
  test
end
...
# def players(brd)
#   { "player" => player_places_piece!(brd), "computer" => computer_places_piece!(brd)}
# end

# def place_piece!(current_player)
#   if current_player == players.keys.first
#     players.fetch("player")
#   else
#     players.fetch("computer")
#   end
# end
...
def player_goes_first?(choice, brd)
  players(brd).keys.first.start_with?(choice)
end
...
def place_piece!(brd, player)
  player_goes_first?(player, brd) == player ? players.fetch("player") : players.fetch("computer")
end

def alternate_player(player, brd)
  player_goes_first?(player, brd) == player ? players.keys.last : players.keys.first
end

def main_game_play(brd, scores)
  current_player = grab_first_player
  
  loop do
    available_squares = joinor(empty_squares(brd)).to_s
    display_board(brd, scores)

    prompt("Choose a position to place a piece: (#{available_squares})")

    place_piece!(brd, current_player)
    current_player = alternate_player(current_player, brd)
...
#players(board).keys.first.start_with?("p")
PLAYERS